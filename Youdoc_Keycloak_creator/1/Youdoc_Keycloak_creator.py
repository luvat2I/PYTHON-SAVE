...
if __name__ == "__main__":
        ...
        if sheet.cell(row=row, column=1).value is not None:
            ...
            try :
                ...
                if YOUDOC_TOKEN_EXPIRE["error"] :
                    ...
                    YOUDOC_TOKEN_EXPIRE_token = YOUDOC_TOKEN_EXPIRE["return"]
                if DEBUG : print(f"> 2 > Token YD : {YOUDOC_TOKEN_EXPIRE_token}")
                ...
                if KEYCLOAK_TOKEN_EXPIRE_token :
                    ...
                    if KEYCLOAK_TOKEN["error"] :
                        ...
                        if DEBUG :
                            print(f"> 2 > Token : {KEYCLOAK_TOKEN["return"]}")
                ...
                if ACTION == "CREATION" :
                    ...
                    KEYCLOAK_ADMIN_MAPPER = KEYCLOAK_CONNECT.admin_mapper(IDP_ALIAS,GROUPE_NAME,ROLE_NAME,MAPPER_ID,"",ACTION)
                ...
                if ACTION == "SUPPRESSION" :
                    ...
                    if KEYCLOAK_ID_MAPPER["error"] :
                        ...
                        KEYCLOAK_ADMIN_MAPPER = KEYCLOAK_CONNECT.admin_mapper(IDP_ALIAS,GROUPE_NAME,ROLE_NAME,MAPPER_ID,KEYCLOAK_ID_MAPPER,ACTION)
                        if KEYCLOAK_ADMIN_MAPPER["error"] :
                            print(KEYCLOAK_ADMIN_MAPPER["return"])
                        else :
                            if DEBUG : print(f"{KEYCLOAK_ADMIN_MAPPER["return"]}")
    ...
    # sys.exit(1)        
            ...
            # if ACTION == "CREATION" :
                # ma_creation = KEYCLOAK_CONNECT.admin_role(ROLE_NAME,ACTION)
                # if ma_creation["error"] :
                    # print(ma_creation["return"])
                    # print("Appuyer sur entrer pour continuer")
                    # input()
                # else :
                    # print(f"> 4 > {ma_creation["return"]}")
            # if ACTION == "SUPPRESSION" :
                # ma_suppression = KEYCLOAK_CONNECT.admin_role(ROLE_NAME,ACTION)
                # if ma_suppression["error"] :
                    # print(ma_suppression["return"])
                    # print("Appuyer sur entrer pour continuer")
                    # input()
                # else :
                    # print(f"> 4 > {ma_suppression["return"]}")
    ...
    # if YOUDOC_TOKEN_EXPIRE["error"] :
        # print(YOUDOC_TOKEN_EXPIRE["return"])
    # else :
        # print(f"{YOUDOC_TOKEN_EXPIRE["return"]}")
        # YOUDOC_TOKEN_EXPIRE_token = YOUDOC_TOKEN_EXPIRE["return"]
    # print(YOUDOC_TOKEN_EXPIRE_token)
    
    # KEYCLOAK_TOKEN_EXPIRE = YOUDOC_CONNEXION.token_valide()
    # if KEYCLOAK_TOKEN_EXPIRE["error"] :
        # print(KEYCLOAK_TOKEN_EXPIRE["return"])
    # else :
        # print(f"{KEYCLOAK_TOKEN_EXPIRE["return"]}")
        # KEYCLOAK_TOKEN_EXPIRE = KEYCLOAK_TOKEN_EXPIRE["return"]
    # print(KEYCLOAK_TOKEN_EXPIRE)
    ...
    # print(f"> 4 > document : {EXCEL_FICHIER}")
    ...
    # print(f"")
    # if ACTION == "CREATION" :
        # print(f"> 4 > Nombre de roles en creation : {nb_role}")
    # if ACTION == "SUPPRESSION" :
        # print(f"> 4 > Nombre de roles en suppression : {nb_role}")
    # print(f"")
