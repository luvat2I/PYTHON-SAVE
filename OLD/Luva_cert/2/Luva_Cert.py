...
def traitementcreation(filename_ini):
	log_event("0","DEBUG",f"Traitement de création de certificats pour '{filename_ini}'.")
	
	log_event("0","DEBUG",f"Recuperation des datas du ini '{filename_ini}'.")
	...
	try:
		...
		log_secure("0","ERROR",f"L'option 'COMMON_NAME' est manquante dans le fichier '{filename_ini}': {e}")
		sys.exit(1)  # Quitte l'application avec un code d'erreur
	...
	try:
		...
		if PROCESSUS == "CREATION" :
			DOSSIER_EXPORT = ""
		if PROCESSUS == "AUTO" :
			log_secure("0","ERROR",f"L'option 'DOSSIER_EXPORT' est manquante dans le fichier '{filename_ini}': {e}")
			sys.exit(1)  # Quitte l'application avec un code d'erreur
	...
	try:
		...
		log_secure("0","ERROR",f"L'option 'NB_BITS' est manquante dans le fichier '{exe_filename}': {e}")
		sys.exit(1)  # Quitte l'application avec un code d'erreur
	...
	try:
		...
		log_secure("0","ERROR",f"L'option 'JOURS' est manquante dans le fichier '{exe_filename}': {e}")
		sys.exit(1)  # Quitte l'application avec un code d'erreur
...
def traitementextraction(filename_ini):
	log_event("0","DEBUG",f"Traitement d'extraction pfx pour '{filename_ini}'.")
	log_event("0","DEBUG",f"Recuperation des datas du ini '{filename_ini}'.")
	...
	filenamecert = f"{DOSSIER_EXPORT}{filename_ini}"
	
	OUT_KEY = filenamecert.replace(".ini", ".key")
	OUT_KEY = OUT_KEY.replace(".INI", ".key")
	OUT_CRT = filenamecert.replace(".ini", ".crt")
	OUT_CRT = OUT_CRT.replace(".INI", ".crt")
	OUT_PFX = filename_ini.replace(".INI", ".pfx")
	OUT_PFX = OUT_PFX.replace(".ini", ".pfx")
	
	log_event("0","DEBUG",f"extraction du fichier PFX '{OUT_PFX}'.")
	pfx_path = OUT_PFX
...
def traitementcacerts(filename_ini):
	log_event("0","DEBUG",f"Traitement de génération de cacerts pour '{filename_ini}'.")
	log_event("0","DEBUG",f"Recuperation des datas du ini '{filename_ini}'.")
	...
	try:
		...
		if PROCESSUS == "CACERTS" :
			DOSSIER_CRT = ""
		if PROCESSUS == "AUTO" :
			log_secure("0","ERROR",f"L'option 'DOSSIER_CRT' est manquante dans le fichier '{filename_ini}': {e}")
			sys.exit(1)  # Quitte l'application avec un code d'erreur
	
	# --- Configuration
	if DOSSIER_EXPORT == "" :
		jks_path_interne = Path(path).parent
	else :
		jks_path_interne = Path(f"{DOSSIER_EXPORT}")
	
	if DOSSIER_EXPORT == "" :
		bak = Path(path).parent / f"cacerts.temp"
	else :
		bak = Path(f"{DOSSIER_EXPORT}cacerts.temp")
		
	if DOSSIER_EXPORT == "" :
		out_path = Path(path).parent / f"cacerts"
	else :
		out_path = Path(f"{DOSSIER_EXPORT}cacerts")
	
	if DOSSIER_EXPORT == "" :
		jks_path = Path(path).parent / f"cacerts.base"
	else :
		jks_path = Path(f"{DOSSIER_EXPORT}cacerts.base")
	jks_path_base = Path(path).parent / f"cacert" / f"cacerts.base"
	if DOSSIER_CRT == "" :
		cert_doss = Path(path).parent
	else :
		cert_doss = Path(f"{DOSSIER_CRT}")
	
	jks_path_interne = f"{DOSSIER_EXPORT}"
	password = PFX_PASSWORD
	
	if DEV:
		if not jks_path_base.exists():
			log_secure("0","ERROR",f"Keystore de base introuvable: {jks_path_base}")
			sys.exit(1)
...
if __name__ == "__main__":
	...
	if PROCESSUS == "CREATION":
		traitementcreation(ini_filename)
	
	if PROCESSUS == "EXTRACTION":
		traitementextraction(ini_filename)
		
	if PROCESSUS == "CACERTS":
		traitementcacerts(ini_filename)
			...
			if file.endswith(fintype_ini.lower()) or file.endswith(fintype_ini):
				...
				try:
					...
					if INI_PROCESSUS == "CREATION":
						traitementcreation(file)
					
					if INI_PROCESSUS == "EXTRACTION":
						traitementextraction(file)
