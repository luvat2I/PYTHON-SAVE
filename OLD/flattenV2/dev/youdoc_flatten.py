...
def count_layers_in_pdf(pdf_path):
    doc = fitz.open(pdf_path)
    layers_count = 0
	ocgs = doc.GetOCGs()
    # Parcourir chaque page pour vérifier les calques
    for page in doc:
        # Vérifier les calques de la page
        if page.get_ocgs():
            layers_count += len(page.get_ocgs())  # Compte le nombre de calques sur la page
...
pdf_path = 'D:\\PYTHON\\flattenV2\\dev\\temporaire\\test2.pdf'  # Remplacez par le chemin de votre fichier PDF
nombre_de_calques = count_layers_in_pdf(pdf_path)

print(f'Nombre de calques dans le PDF : {nombre_de_calques}')