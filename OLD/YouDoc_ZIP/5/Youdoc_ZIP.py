...
if __name__ == "__main__":
    ...
    logaffiche = "0 > Lecture des parametres en cours "
    print(f"{logaffiche}...")
    ...
    try:
        ...
        fait_etape = config['PARAM']['fait_etape']
    ...
    logaffiche = "0 > test de la connexion SQL "
    print(f"{logaffiche}...")
    try:
    ...
    if fait_etape == "0" : 
        print(f"fait_etape ... {fait_etape}")
        input()
        sys.exit(1)
        
    
    ### Récupération de la liste des ZIP 
    logaffiche = "1 > Decompress ZIP (0)"
    print(f"{logaffiche} >")
    ...
    while reste_zip :
        ...
        for row in return_zip_document:
            ...
            print(f"{logaffiche} > {ID_DOCUMENT_ZIP}")
            ...
            try :
                ...
                print(f"{logaffiche} > {ID_DOCUMENT_ZIP} > OK (0 > 1)")
            except Exception as e:
                db_query_maj_document = f"update [{sgbd_database}].[dbo].[{sgbd_data}] set STATUS = 9 where [{sgbd_database}].[dbo].[{sgbd_data}].[object_id_zip] = '{FILE_DOCUMENT_ZIP}'"
                return_zip_document_maj = db.execute_query_save(db_query_maj_document)
                print(f"{logaffiche} > {ID_DOCUMENT_ZIP} > {e} (0 > 9)")
    ...
    while reste_zip :
        ...
        for row in return_zip_document:
            ...
            for file in os.listdir(DOCUMENT_ZIP_TEMP) :
                print(f"{logaffiche} > {ID_DOCUMENT_ZIP} > {file}")
                
                extension = Youdoc_ZIP_complement.get_extension(file)
                
                ID_type = "2"
                if extension == "AUCUN" :
                    ID_type = "9"
                
                db_query_list_document = f"insert into [{sgbd_database}].[dbo].[{sgbd_data}] SELECT top 1 [object_id_zip], '', '{file}', '{extension}', [path_and_file_name] ,{ID_type} FROM [{sgbd_database}].[dbo].[{sgbd_data}] where object_id_zip = '{row.object_id_zip}'"
                return_list_document_maj = db.execute_query_save(db_query_list_document)
                print(f"{logaffiche} > {ID_DOCUMENT_ZIP} > {file} > (2)")
            ...
            return_zip_document_maj = db.execute_query_save(db_query_maj_document)
            print(f"{logaffiche} > {ID_DOCUMENT_ZIP} > OK (1 > 3)")
    ...
    logaffiche = "3 > Nom def doc (2)"
    print(f"{logaffiche} > ")
    
    try: 
        db_query_max = f"SELECT max(object_id) as max_valeur FROM [{sgbd_database}].[dbo].[{sgbd_data}]"
        return_max = db.execute_query(db_query_max)

        for row in return_max:
            ID_DOCUMENT = row.max_valeur
            ID_DOCUMENT = Youdoc_ZIP_complement.supprime_extension(ID_DOCUMENT)
    except :
        ID_DOCUMENT = "0AAAAAAA"
    if ID_DOCUMENT == "" : ID_DOCUMENT = "0AAAAAAA"
    print(f"{logaffiche} > recup id > {ID_DOCUMENT}")
    ...
    while reste_zip :
        ...
        for row in return_zip_document:
            ...
            try :
                ...
                EXTENSION_DOCUMENT = row.media_type
                
                print(f"{logaffiche} > {FILE_DOCUMENT} > {ID_DOCUMENT}")
                ...
                FILE_DEF = DOCUMENT_ZIP_DATA.replace(row.object_id_zip,"") + ID_DOCUMENT + "." + EXTENSION_DOCUMENT
                
                db_query_maj_document = f"update [{sgbd_database}].[dbo].[{sgbd_data}] set STATUS = 4,object_id = '{ID_DOCUMENT}.{EXTENSION_DOCUMENT}'  where [{sgbd_database}].[dbo].[{sgbd_data}].[object_id_zip] = '{FILE_DOCUMENT_ZIP}' and [{sgbd_database}].[dbo].[{sgbd_data}].[object_name] = '{row.object_name}'"
                return_zip_document_maj = db.execute_query_save(db_query_maj_document)
                
                print(f"{logaffiche} > {FILE_DOCUMENT} > {ID_DOCUMENT} > Deplace OK (2 > 4)")
                
            except Exception as e:
                print(f"{logaffiche} > {FILE_DOCUMENT} > {ID_DOCUMENT} > {e}")
                db_query_maj_document = f"update [{sgbd_database}].[dbo].[{sgbd_data}] set STATUS = 9  where [{sgbd_database}].[dbo].[{sgbd_data}].[object_id_zip] = '{FILE_DOCUMENT_ZIP}' and [{sgbd_database}].[dbo].[{sgbd_data}].[object_name] = '{row.object_name}'"
                return_zip_document_maj = db.execute_query_save(db_query_maj_document)
                print(f"{logaffiche} > {FILE_DOCUMENT} > {ID_DOCUMENT} > (2 > 9)")
    ...
    while reste_zip :
        ...
        for row in return_zip_document:
            ...
            print(f"{logaffiche} > {FILE_object_id}")
            ...
            print(f"{logaffiche} > {FILE_object_id} > ARAPREP")
            # ARARREP
            ...
            return_create_data = db.execute_query_save(db_query_create_data)
            print(f"{logaffiche} > {FILE_object_id} > ARARREP")
            ...
            return_create_data = db.execute_query_save(db_query_create_data)
            print(f"{logaffiche} > {FILE_object_id} > ecm_daddl")
            # OP8wT
            ...
            return_create_data = db.execute_query_save(db_query_create_data)
            print(f"{logaffiche} > {FILE_object_id} > OP8wT")
            # ARRAUREP
            ...
            return_create_data = db.execute_query_save(db_query_create_data)
            print(f"{logaffiche} > {FILE_object_id} > ARRAUREP")
            ...
            return_create_data = db.execute_query_save(db_query_create_data)
            print(f"{logaffiche} > {FILE_object_id} > OST_DOCUMENT")
            #ARG0CP
            ...
            return_create_data = db.execute_query_save(db_query_create_data)
            print(f"{logaffiche} > {FILE_object_id} > ARG0CP")
            #update
            ...
            return_zip_document_maj = db.execute_query_save(db_query_maj_document)
            print(f"{logaffiche} > {FILE_object_id} > OK (4 > 5)")
    ...
    logaffiche = "5 > Data def doc (5)"
    print(f"{logaffiche} > ")
    
    db_query_count = f"SELECT count(*) FROM [{sgbd_database}].[dbo].[{sgbd_data}] where STATUS = 5"
    return_deplace_document_count = db.execute_query_count(db_query_count)
    if return_deplace_document_count == 0 : reste_zip = False
    else : reste_zip = True
    
    
    while reste_zip :
        
        db_query_deplace_document = f"SELECT TOP (1000) * FROM [{sgbd_database}].[dbo].[{sgbd_data}] where STATUS = 5"
        return_deplace_document_count = db.execute_query(db_query_deplace_document)
        reste_zip = False
        for row in return_deplace_document_count:
            ...
            try :
                ...
                FILE_TEMP = DOCUMENT_ZIP_TEMP + r"\\" + row.object_name
                FILE_DEF = DOCUMENT_ZIP_DATA.replace(row.object_id_zip,"") + row.object_id
                
                zip_class.deplace_fic_rename(FILE_TEMP,FILE_DEF)
                
                db_query_maj_document = f"update [{sgbd_database}].[dbo].[{sgbd_data}] set STATUS = 5 where [{sgbd_database}].[dbo].[{sgbd_data}].[object_id_zip] = '{FILE_DOCUMENT_ZIP}' and [{sgbd_database}].[dbo].[{sgbd_data}].[object_name] = '{row.object_name}'"
                return_zip_document_maj = db.execute_query_save(db_query_maj_document)
                
                print(f"{logaffiche} > {FILE_DOCUMENT} > {ID_DOCUMENT} > Deplace OK (4 > 5)")
                
            except Exception as e:
                print(f"{logaffiche} > {FILE_DOCUMENT} > {ID_DOCUMENT} > {e}")
                db_query_maj_document = f"update [{sgbd_database}].[dbo].[{sgbd_data}] set STATUS = 9  where [{sgbd_database}].[dbo].[{sgbd_data}].[object_id_zip] = '{FILE_DOCUMENT_ZIP}' and [{sgbd_database}].[dbo].[{sgbd_data}].[object_name] = '{row.object_name}'"
                return_zip_document_maj = db.execute_query_save(db_query_maj_document)
                print(f"{logaffiche} > {FILE_DOCUMENT} > {ID_DOCUMENT} > (4 > 9)")
