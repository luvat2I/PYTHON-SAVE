...
if __name__ == "__main__":
	...
	if traitement_type == "DOCUMENT":
		...
		for i in range(0, int(db_results_count), int(traitement_lot)):
			if int(traitement_lot) >= int(db_results_count) :
				calcule_range_min = 0
				calcule_range_max = int(db_results_count)
			else :
			...
			log_aff.log_base("INFO",f"traitement du lot {i} > de {calcule_range_min} à {calcule_range_max} sur {db_results_count}")
			...
			for row in db_results:
				...
				if traitement_maj_index :
					if sgbd_db_type == 'DB2':
						...
						db_query_save = f"UPDATE ECM_IDX_F set STATUS = '{retour}' where FOLDERID = '{row.FOLDERID}' WITH NC"
						db_query_save.upper()
					else :
						log_aff.log_base("INFO",f"ID dossier > {row.folderId} > ECM_IDX_F à {retour}")
