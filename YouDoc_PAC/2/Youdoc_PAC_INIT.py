...
if __name__ == "__main__":
	...
	log_event("0","INFO",f"SGBD > Dévut du traitement de création des databases")
	...
	log_event("0","INFO",f" Fin du traitement ajout des DATABASES.")
	
	log_event("0","INFO",f" Début du traitement ajout des MULTI.")
				...
				for E_FLD in L_FLD.findall('E_FLD'):
					for L_TYP in E_FLD.findall('L_TYP'):
						for E_TYP in L_TYP.findall('E_TYP'):
							...
							if E_TYPmul == "2" :
								if sgbd_db_type == 'DB2':
									log_event("0","DEBUG",f" update {sgbd_database}.ECM_KITYDO set fldIdxTyp = 'MULTI' where kindid = '{E_TYPname}'")
									query_ECM_PARAM = f"update {sgbd_database}.ECM_KITYDO set FLDIDXTYP = 'MULTI' where KINDID = '{E_TYPname}' "
									db.execute_query_save(query_ECM_PARAM)
								else :
									log_event("0","DEBUG",f" update ECM_KITYDO set fldIdxTyp = 'MULTI' where kindid = '{E_TYPname}'")
									query_ECM_PARAM = f"update ECM_KITYDO set fldIdxTyp = 'MULTI' where kindid = '{E_TYPname}' "
									db.execute_query_save(query_ECM_PARAM)
	...
	for Environment in root.findall('Environment'):
		for Base in Environment.findall('Base'):
			for L_FLD in Base.findall('L_FLD'):
				for E_FLD in L_FLD.findall('E_FLD'):
					for L_TYP in E_FLD.findall('E_SUB'):
						for L_TYP in E_FLD.findall('L_TYP'):
							for E_TYP in L_TYP.findall('E_TYP'):
								E_TYPname = E_TYP.get('nme')
								E_TYPmul = E_TYP.get('mul')
	...
	log_event("0","INFO",f" Fin du traitement ajout des MULTI.")
