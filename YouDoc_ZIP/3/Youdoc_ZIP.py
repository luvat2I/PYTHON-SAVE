...
if __name__ == "__main__":
	...
	print(f"{logaffiche} > recup OP5oA")
	...
	print(f"{logaffiche} > recup OP5oA > {ID_DOCUMENT}")
	...
	while reste_zip :
		...
		for row in return_zip_document:
			...
			try :
				...
				# save au cas où
				ID_DOCUMENT_suivant = Youdoc_ZIP_complement.ID_suivant(ID_DOCUMENT)
				db_query_maj_ID = f"update [{sgbd_database}].[dbo].[OPb5T] set OP5oA = '{ID_DOCUMENT_suivant}'"
	...
	print(f"{logaffiche} > OP5oA > {ID_DOCUMENT}")
	db_query_maj_ID = f"update [{sgbd_database}].[dbo].[OPb5T] set OP5oA = '{ID_DOCUMENT}'"
	return_zip_document_ID = db.execute_query_save(db_query_maj_ID)
	print(f"{logaffiche} > OP5oA > {ID_DOCUMENT} > ok")
