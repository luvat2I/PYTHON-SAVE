...
def luva_lic_calcul(type,annee,mois,jour,duree,DEV):
	...
	if type == "DUREE" :
		NB_DUREE_0 = duree + nombreW + nombreX + nombreY + nombreZ + 10
	else :
	...
	if type == "AN" or  type == "MOIS" or  type == "JOURS" or  type == "DUREE" :	
		...
		LIC_AN_1 = ANNEE_1 + nombreW + nombreX * 2
		LIC_AN_2 = ANNEE_2 + nombreW + nombreY * 3
		LIC_AN_3 = ANNEE_3 + nombreW + nombreZ * 2
		LIC_AN_4 = ANNEE_4 + nombreY + nombreZ * 3 
...
def valide_licence(LICENCE_CLIENT,DEV):
	...
	if DEV : print(f"> LUVA > MODEDEV > DATE_TODAY = {DATE_TODAY}")
	if DEV : print(f"> LUVA > MODEDEV > LICENCE = {LICENCE_CLIENT}")
	
	NB_DUREE_1 = chiffre_lic(0,LICENCE_CLIENT)
	
	if DEV : print(f"> LUVA > MODEDEV >  Savoir si DUREE > {NB_DUREE_1}")
	if NB_DUREE_1 > 5 :
		TYPE = "DUREE"
	else :
		TYPE = "AUTRE"
	if DEV : print(f"> LUVA > MODEDEV > TYPE > {TYPE}")
	
	nombre_inutil = chiffre_lic(1,LICENCE_CLIENT)
	if DEV : print(f"> LUVA > MODEDEV > nombre_inutil > {nombre_inutil}")
	
	NB_DUREE_2 = chiffre_lic(2,LICENCE_CLIENT)
	if DEV : print(f"> LUVA > MODEDEV > NB_DUREE_2 > {NB_DUREE_2}")
	
	nombreW = chiffre_lic(3,LICENCE_CLIENT)
	if DEV : print(f"> LUVA > MODEDEV > nombreW > {nombreW}")
	
	nombreX = chiffre_lic(4,LICENCE_CLIENT)
	if DEV : print(f"> LUVA > MODEDEV > nombreX > {nombreX}")
	
	nombreY = chiffre_lic((len(str(LICENCE_CLIENT))-2),LICENCE_CLIENT)
	if DEV : print(f"> LUVA > MODEDEV > nombreY > {nombreY}")
	
	nombreZ = chiffre_lic((len(str(LICENCE_CLIENT))-1),LICENCE_CLIENT)
	if DEV : print(f"> LUVA > MODEDEV > nombreZ > {nombreZ}")
	...
	if TYPE == "DUREE" and NB_DUREE_2 < 5:
		...
		if DEV : print(f"> LUVA > MODEDEV > NB_DUREE_3 > {NB_DUREE_3}")
	if TYPE == "DUREE" and NB_DUREE_2 > 5:
	...
	if TYPE == "DUREE" :
		...
		if DEV : print(f"> LUVA > MODEDEV > ANNEE_DEBUT > {ANNEE_DEBUT}")
	...
	if TYPE == "AUTRE" :
		...
		if DEV : print(f"> LUVA > MODEDEV > LICENCE_AN : {LICENCE_AN}")
		...
		if DEV : print(f"> LUVA > MODEDEV > LICENCE_MOIS : {LICENCE_MOIS}")
		...
		if DEV : print(f"> LUVA > MODEDEV > LICENCE_JOUR : {LICENCE_JOUR}")
	...
	if not VALIDE :
		...
		if DEV : print(f"> LUVA > MODEDEV > LICENCE_DUREE : {LICENCE_DUREE}")
