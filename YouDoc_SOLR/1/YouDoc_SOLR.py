...
try:
	db_data_liste = config['SGBD']['db_data_liste']
except Exception as e:
	log_secure("0","ERROR",f"L'option 'db_data_liste' est manquante dans le fichier 'config.ini': {e}")
	exit(1)  # Quitte l'application avec un code d'erreur

try:
	db_data_status = config['SGBD']['db_data_status']
except Exception as e:
	log_secure("0","ERROR",f"L'option 'db_data_status' est manquante dans le fichier 'config.ini': {e}")
	exit(1)  # Quitte l'application avec un code d'erreur

try:
	db_a_suppr = config['SGBD']['db_a_suppr']
except Exception as e:
	log_secure("0","ERROR",f"L'option 'db_a_suppr' est manquante dans le fichier 'config.ini': {e}")
	exit(1)  # Quitte l'application avec un code d'erreur


try:
	db_fait = config['SGBD']['db_fait']
except Exception as e:
	log_secure("0","ERROR",f"L'option 'db_fait' est manquante dans le fichier 'config.ini': {e}")
	exit(1)  # Quitte l'application avec un code d'erreur

try:
	db_data_lot = config['SGBD']['db_data_lot']
except Exception as e:
	log_secure("0","ERROR",f"L'option 'db_data_lot' est manquante dans le fichier 'config.ini': {e}")
	exit(1)  # Quitte l'application avec un code d'erreur
	
try:
	solr_hostname = config['SOLR']['solr_hostname']
except Exception as e:
	log_secure("0","ERROR",f"L'option 'solr_hostname' est manquante dans le fichier 'config.ini' : {e}")
	sys.exit(1)  # ferme lapp

try:
	solr_username = config['SOLR']['solr_username']
except Exception as e:
	log_secure("0","ERROR",f"L'option 'solr_username' est manquante dans le fichier 'config.ini' : {e}")
	sys.exit(1)  # ferme lapp

try:
	solr_password = config['SOLR']['solr_password']
except Exception as e:
	log_secure("0","ERROR",f"L'option 'solr_password' est manquante dans le fichier 'config.ini' : {e}")
	sys.exit(1)  # ferme lapp

try:
	solr_tenant = config['SOLR']['solr_tenant']
except Exception as e:
	log_secure("0","ERROR",f"L'option 'solr_tenant' est manquante dans le fichier 'config.ini' : {e}")
	sys.exit(1)  # ferme lapp

try:
	solr_requete = config['SOLR']['solr_requete']
except Exception as e:
	log_secure("0","ERROR",f"L'option 'solr_requete' est manquante dans le fichier 'config.ini' : {e}")
	sys.exit(1)  # ferme lapp
...
if __name__ == "__main__":
	...
	log_event("0","DEBUG",f"DATA > bdd : {sgbd_database} table : {db_data} liste : {db_data_liste} status : {db_data_status} a supp : {db_a_suppr} lot : {db_data_lot} ")
	log_event("0","DEBUG",f"SOLR > Solr : {solr_hostname} user : {solr_username} mdp : {solr_password} tenant : {solr_tenant} ")
	log_event("0","DEBUG",f"SOLR > Requete : {solr_requete}")
	...
	log_event("0","INFO",f"SGBD > Lecture de la liste")
	
	if sgbd_db_type == 'DB2':
		db_query_liste = f"SELECT {db_data_liste} FROM {sgbd_database}.{db_data} WHERE {db_data_status} = '{db_a_suppr}' fetch first {db_data_lot} rows only"
		db_query_liste.upper()
	else :
		db_query_liste = f"SELECT TOP {db_data_lot} {db_data_liste} FROM {db_data} WHERE {db_data_status} = '{db_a_suppr}'"
	
	if sgbd_db_type == 'DB2':
		db_query_count = f"SELECT count(*) FROM {sgbd_database}.{db_data} WHERE {db_data_status} = '{db_a_suppr}'"
		db_query_count.upper()
	else :
		db_query_count = f"SELECT count(*) FROM {db_data} where {db_data_status} = '{db_a_suppr}'"
	
	
	
	
	log_event("0","DEBUG",f"SGBD > QUERY > {db_query_count}")
	db_results_count = db.execute_query_count(db_query_count)
	log_event("0","DEBUG",f"SGBD > QUERY > {db_query_count} > RESULT > {db_results_count}")
	...
	for i in range(0, int(db_results_count), int(db_data_lot)):
	
		log_event("0","DEBUG",f"SGBD > QUERY > {db_query_liste}")
		db_results_liste = db.execute_query(db_query_liste)
		
		for row in db_results_liste:
			log_event("0","DEBUG",f"SOLR > TRAITEMENT > {row[0]}")
			
			solr_query = f"id:{solr_requete}{solr_tenant}-{row[0]}"
			log_event("0","DEBUG",f"SOLR > REQUETE > {solr_query}")
			
			client.delete(solr_query)
		...
		for row in db_results_liste:
			...
			log_event("0","DEBUG",f"SOLR > VERIF > {retour}")
			
			if sgbd_db_type == 'DB2':
				db_query_save = f"UPDATE {sgbd_database}.{db_data} set {db_data_status} = '{db_fait}' where {db_data_liste} = '{row[0]}' WITH NC"
				db_query_save.upper()
			else :
				db_query_save = f"UPDATE {db_data} set {db_data_status} = '{db_fait}' where {db_data_liste} = '{row[0]}';"
