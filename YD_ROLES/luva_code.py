...
def log_enreg(LOG_LEVEL,LOG_TEXT,LOGGER_FOLDER,LOG_FOLDER_LEVEL):
	current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def log_event(LOG_TYPE,LOG_LEVEL,LOG_TEXT,LOG_EVENT_LEVEL,LOG_CONSOLE_LEVEL,LOG_FOLDER_LEVEL,LOGGER_FOLDER,LOGGER_SERVICE,TRAITEMENT_TYPE,ENABLE_LOGGING):
	if LOG_TYPE == "0" :
		if ENABLE_LOGGING :
			log_enreg(LOG_LEVEL,LOG_TEXT,LOGGER_FOLDER,LOG_FOLDER_LEVEL)
		log_service(LOG_LEVEL,LOG_TEXT,LOG_EVENT_LEVEL,LOGGER_SERVICE)
		log_console(LOG_LEVEL,LOG_TEXT,LOG_CONSOLE_LEVEL,TRAITEMENT_TYPE)
	elif LOG_TYPE == "1" :
		if ENABLE_LOGGING :
...
def get_enable_logging(LOG_VALIDATION,CONFIG_INI,MODE_DEV):
	if MODE_DEV : print(f"get_enable_logging > LOG_VALIDATION : {LOG_VALIDATION}")
	try:
		if LOG_VALIDATION :
			enable_logging = CONFIG_INI.getboolean("logging", "enable_logging")
			if MODE_DEV : print(f"get_enable_logging > enable_logging : {enable_logging}")
		else :
			enable_logging = False
	except Exception as e:
		if MODE_DEV : print(f"ERROR get_enable_logging > {e}")
		enable_logging = False	
