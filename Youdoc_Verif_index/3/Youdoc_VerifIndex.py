...
if __name__ == "__main__":
    ...
    print(f"> 1 > Liste information solr sur le tenant {SOLR_tenant}")
    ...
    DB_data_fold = luva_code.get_param(config,'FOLDER','DB_data_fold')
    if DB_data_fold["error"] :
        print(DB_data_fold["return"])
        print("Appuyer sur entrer pour fermer")
        input()
        sys.exit(1)
    else :
        DB_data_fold = DB_data_fold["return"]
        
    DB_name_fold = luva_code.get_param(config,'FOLDER','DB_name_fold')
    if DB_name_fold["error"] :
        print(DB_name_fold["return"])
        print("Appuyer sur entrer pour fermer")
        input()
        sys.exit(1)
    else :
        DB_name_fold = DB_name_fold["return"]

        
    DB_name_status = luva_code.get_param(config,'FOLDER','DB_name_status')
    if DB_name_status["error"] :
        print(DB_name_status["return"])
        print("Appuyer sur entrer pour fermer")
        input()
        sys.exit(1)
    else :
        DB_name_status = DB_name_status["return"]
        
    Lot_fold = luva_code.get_param(config,'FOLDER','Lot_fold')
    if Lot_fold["error"] :
        print(Lot_fold["return"])
        print("Appuyer sur entrer pour fermer")
        input()
        sys.exit(1)
    else :
        Lot_fold = Lot_fold["return"]

    DB_data_docactive = luva_code.get_bool_param(config,'DOCUMENT','DB_data_docactive')
    if DB_data_docactive["error"] :
        print(DB_data_docactive["return"])
        print("Appuyer sur entrer pour fermer")
        input()
        sys.exit(1)
    else :
        DB_data_docactive = DB_data_docactive["return"]
    
    DB_name_docfold = luva_code.get_param(config,'DOCUMENT','DB_name_docfold')
    if DB_name_docfold["error"] :
        print(DB_name_docfold["return"])
        print("Appuyer sur entrer pour fermer")
        input()
        sys.exit(1)
    else :
        DB_name_docfold = DB_name_docfold["return"]
        
        
    
    DB_name_docstatus = luva_code.get_param(config,'DOCUMENT','DB_name_docstatus')
    if DB_name_docstatus["error"] :
        print(DB_name_docstatus["return"])
        print("Appuyer sur entrer pour fermer")
        input()
        sys.exit(1)
    else :
        DB_name_docstatus = DB_name_docstatus["return"]
        
        
        
    DB_data_doc = luva_code.get_param(config,'DOCUMENT','DB_data_doc')
    if DB_data_doc["error"] :
        print(DB_data_doc["return"])
        print("Appuyer sur entrer pour fermer")
        input()
        sys.exit(1)
    else :
        DB_data_doc = DB_data_doc["return"]
        
    DB_name_doc = luva_code.get_param(config,'DOCUMENT','DB_name_doc')
    if DB_name_doc["error"] :
        print(DB_name_doc["return"])
        print("Appuyer sur entrer pour fermer")
        input()
        sys.exit(1)
    else :
        DB_name_doc = DB_name_doc["return"]
    ...
    print(f"> 1 > Pour les folders informations dans {DB_database}.dbo.{DB_data_fold} dans {DB_name_fold} par lot de {Lot_fold}")
    print(f"> 1 > Pour les documents informations dans {DB_database}.dbo.{DB_data_doc} dans {DB_name_doc}")
    print(f"")
    print(f"> 1 > Récuperation des paramètres du fichier {ini_filename} > FINI")
    print(f"")
   
    print(f"> 2 > Connexion SQL ")
    ...
    if return_db["error"] == "" : 
        print(f"> 2 > {return_db["result"]}")
    else :
        print(f"> 2 > connexion {return_db["result"]} {return_db["message"]}")
        db.close()
        input()
        sys.exit(1)
    print(f"> 2 > Connexion SQL > FIN")
    print(f"")
    
    db_query_top = f"SELECT TOP ({Lot_fold}) {DB_name_fold} FROM [{DB_database}].[dbo].[{DB_data_fold}] WITH (NOLOCK) WHERE {DB_name_status} = '' "
    
    ClientSolr = solr_class.SolrConnection(SOLR_hostname,SOLR_username,SOLR_password,SOLR_tenant,VERIF_SSL,is_debug)
    
    if is_debug : print(f"> DEBUG > {db_query_top}")
    returndb_top = db.execute_query(db_query_top)
    if returndb_top == "ERROR" :
        print(f"> Une erreur de lecture de {db_query_top} à eu lieu")
        boucle = False
    else :
        if not returndb_top:
            boucle = False
            ...
            boucle = True
    while boucle :
        for row in returndb_top:
            try :
                print(f"> {row.EZBUNU} > ")
                if DB_data_docactive :
                    solr_query = f"XECM.Document.folders.item.id:{row.EZBUNU}"
                    retour = ClientSolr.SolrListeID_Document_Folder(solr_query)
                    if retour["error"] :
                        print(retour["return"])
                        input()
                    else :
                        ...
                        if not ids:
                            if is_debug : print("Pas de doc")
                        # Insérer les ID dans la base de données
                        for doc in ids:
                            id_value = doc['id'].replace(f"XECM-DOCUMENT--{SOLR_tenant}-","")
                            db_query = f"INSERT INTO [{DB_database}].[dbo].[{DB_data_doc}] ({DB_name_docfold},{DB_name_doc},{DB_name_docstatus}) VALUES ('{row.EZBUNU}','{id_value}','I')"
                            print(f"> {row.EZBUNU} > {id_value}")
                            returndbdoc = db.execute_query_save(db_query)
                            if returndbdoc == "ERROR" :
                                print(f"> {id_value} > Une erreur d'enregistrement à eu lieu")
                                input()
                solr_query = f"id:XECM-FOLDER--{SOLR_tenant}-{row.EZBUNU}"
                retour = ClientSolr.SolrVerifFOLDER(solr_query)
                if retour["error"] :
                    db_query = f"update [{DB_database}].[dbo].[{DB_data_fold}] set {DB_name_status} = 'N' Where {DB_name_fold} = '{row.EZBUNU}' "
                else : 
                    db_query = f"update [{DB_database}].[dbo].[{DB_data_fold}] set {DB_name_status} = 'I' Where {DB_name_fold} = '{row.EZBUNU}' "
                returndbfold = db.execute_query_save(db_query)
                if returndbfold == "ERROR" :
                    print(f"> {row.EZBUNU} > Une erreur d'enregistrement à eu lieu")
            except Exception as e:
                print(f"> ERROR > {e}")
                input()
